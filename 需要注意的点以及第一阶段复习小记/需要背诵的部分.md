...
### 16.3 一个奇怪的问题如果foreach里用splice 下一个运算能正常遍历码
    ...
    return function(...args){
        if(args.length>=length){
           return fn(...args)
            ...
            currying(fn.bind(null,...args),length-args.length)
        }
    }
}
...
### ！！object属性如果不是字符和数字 会执行toString这里好好注意
